 ...
 * [OUTPUT]: Provides the compact onboarding Agent list: installed confirmation or an install action.
 * [POS]: Installation-only onboarding surface; detailed availability remains in Agent Settings.
 */
  ...
  return <div role="group" aria-label={backend.displayName} aria-busy={checking || waiting} className="py-3">
    ...
    {feedback && <div className="pl-7 pt-2"><SetupFeedbackNotice feedback={feedback} disabled={busy}
      onRetry={() => void (feedback.kind === "clipboard" || feedback.operation === "check"
        ? setup.recheckBackend(backend.id) : setup.terminalAction(backend.id, "install"))} /></div>}
  </div>;
}
