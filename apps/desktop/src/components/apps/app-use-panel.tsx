 ...
 * [OUTPUT]: Provides separate Conversation and History surfaces with deterministic focus return, Back, New Chat, retry, frozen incremental history, switching recovery, bottom dock, and consistent 32px panel chrome
...
function HistorySurface({ chat }: { chat: AppUseChat }) {
  ...
  if (chat.historyState === "error") {
    return (
      <div className="grid size-full place-items-center gap-3 p-6 text-center text-sm">
        ...
        <Button className="min-h-11" onClick={chat.retryHistory} variant="outline">
  ...
  return (
    ...
    >
      {chat.historyStale && (
        <div className="mb-2 flex min-h-11 items-center gap-2 rounded-lg bg-muted px-3 text-sm" role="status">
            ...
            >
              <span className="flex w-full items-center gap-2">
                ...
                <time className="shrink-0 text-muted-foreground text-xs" dateTime={new Date(item.updatedAt).toISOString()}>
                  {new Intl.DateTimeFormat(i18n.language, { month: "short", day: "numeric" }).format(item.updatedAt)}
                </time>
