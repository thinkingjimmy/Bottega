 ...
 * [OUTPUT]: Desktop mirror artifact previews, native file actions and persistent browser views.
...
export function MirrorArtifacts({ chatId, incarnationId, locale, children, openPanel }: { chatId: string; incarnationId: string; locale: string; children: ReactNode; openPanel?(fence: ArtifactFence, followUp?: ArtifactHost["followUp"]): void }) {
  ...
  const browser = useBrowserTabs(browserOpen), copy = remoteCopy(locale);
  useEffect(() => {
    const lifetime = new AbortController();
    void import("./prose-links").then(({ registerArtifactProseLinks }) => {
      registerArtifactProseLinks(url => {
        void openArtifactBrowser(url, () => setBrowserOpen(true)).catch(console.warn);
  ...
  const host = useMemo<ArtifactHost>(() => {
    ...
    return { scope: `${chatId}:${incarnationId}`, locale, desktop: true,
      ...
      open: async (fence, followUp) => {
        if (fence.kind === "claude-artifact" && fence.url) await openArtifactBrowser(fence.url, () => setBrowserOpen(true));
        else if (openPanel) openPanel(fence, followUp);
        else setPreview({ fence, followUp });
      },
    };
  }, [chatId, incarnationId, locale, openPanel]);
  return <ArtifactHostProvider value={host}>{children}<Dialog open={preview !== null || browserOpen} onOpenChange={open => { if (!open) { setPreview(null); setBrowserOpen(false); } }}>
