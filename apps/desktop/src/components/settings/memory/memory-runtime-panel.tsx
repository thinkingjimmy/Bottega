/**
 * [INPUT]: Depends on React hooks, lucide icons, shared MemoryRuntimeSnapshot/descriptor/target/config-panel agreement, settings-layout SettingsButton/SettingsIconButton/SettingsAlert, MemoryVersionPicker, reveal IPC, i18n and ConfirmationDialog/SlimScroller from @ai-chat/ui
 * [OUTPUT]: Provides MemoryRuntimePanel (the per-engine management drawer: alerts, live operation progress, then either the install pitch or Version/Extraction model/Runtime rows with data-location beside version selection, refresh first in runtime controls, and confirmation before healthy-state repair; plus the logs/version pickers) and MemoryOperationProgress; the config and uninstall dialogs live in memory-runtime-dialogs
 * [POS]: The body of the settings/memory engine drawer; it renders one engine's state and actions only, engine selection belongs to memory-engine-list and the product-level toggle to the view
 */
