 ...
 * [OUTPUT]: Provides once-initialized per-Chat drafts, selection generations, and receipt-authorized submission consumption
...
export function beginAgentSelection(chatId: string, backend: AgentBackendId) {
  ...
  if (state.pending?.submitting) throw new Error("AGENT_SWITCH_SUBMITTING");
  const generation = state.generation + 1;
  const canonical = state.canonical;
  updateAgentDraft(chatId, () => ({ ...state, generation, initialized: true,
...
export function clearAgentDraft(chatId: string) {
  if (readAgentDraft(chatId).pending?.submitting) return;
  drafts.delete(chatId); watchers.forEach(listener => listener());
}
export function markAgentSubmission(chatId: string, intentId: string, generation: number, draftIdentity?: unknown) {
  updateAgentDraft(chatId, state => state.pending?.generation === generation
    ? { ...state, pending: { ...state.pending, submitting: intentId, draftIdentity } } : state);
}
