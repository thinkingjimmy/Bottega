 ...
 * [OUTPUT]: Provides the projects.json v5 schemas with monotonic commit generation, migration parser, intent-bound deletion checkpoints/receipts, and resource-admission types
 * [POS]: Project persistence schema boundary; ProjectStore owns mutation while this module owns validation and v4 migration
 */
...
export const PROJECT_STORE_SCHEMA_VERSION = 5;
...
export const storedProjectSchema = z
  ...
  .superRefine((project, context) => {
    ...
    const invalidAdmission = project.deletionCheckpoint
      ? project.resourceAdmissions.some((item) =>
      ...
      : project.resourceAdmissions.some(
          (item) => item.projectLifecycleRevision !== project.projectLifecycleRevision
        );
    if (invalidAdmission) {
      context.addIssue({
        code: "custom",
