/**
 * [INPUT]: Depends on node: crypto and shared wire mutation operationsAuthority facts provided by BaseOwnerResolver/BasesService in main
 * [OUTPUT]: Provides non-sequenced BaseCommitAuthority, app-gui scope fence, one-time renderer lease and layered scope/revision scanning
 * [POS]: The core of the bases' ability to submit; Wire only has an opaque lease id, real capability only has main memory
 */
...
export class BaseCommitAuthorityRegistry {
  private readonly rendererLeases = new Map<string, RendererLease>();
  ...
  issueRenderer(input: Omit<BaseCommitAuthority, typeof AUTHORITY | "authorityId" | "actor">) {
    const authority = this.create({ ...input, actor: "renderer" });
    const leaseId = randomUUID();
    this.rendererLeases.set(leaseId, {
      leaseId,
      authority,
  ...
  consumeRenderer(leaseId: string) {
    const lease = this.rendererLeases.get(leaseId);
    this.rendererLeases.delete(leaseId);
    if (!lease || lease.expiresAt < this.now()) {
      throw statusError(403, "Base commit authority 无效或已过期");
    }
