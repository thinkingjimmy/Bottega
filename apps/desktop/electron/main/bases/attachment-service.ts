/**
 * [INPUT]: Depends on BaseStore only submit kernel, attachment/support, pure rules, shared attachment/Gallery DTO, thumbnail cache, owner-native/manual and canonical chat identity, admission/event narrow ports
 * [OUTPUT]: Provides owner-native manual upload, automatic access to the Chat library, accessory reading, complete immutable occurrence, goal Chat Gallery
 * [POS]: the business area of the bases; BasesService is only available as an IPC/common line gateway, and this module is exclusive to attachment
 */
...
export class BaseAttachmentService {
  ...
  async readAttachmentForOwner(ownerKey: string, attachmentId: string) {
    ...
    const value = snapshot.rows
      ...
      .find(
        (candidate): candidate is BaseAttachmentValue =>
          isBaseAttachmentValue(candidate) &&
  ...
  async readAttachment(input: ReadAttachmentInput): Promise<ReadAttachmentResult> {
    try {
  ...
  private async ingestAttachment(input: {
    ...
    const committed = await this.store.transactGallery(
      ...
      async ({ snapshot, gallery }) => {
        ...
        const existingRow = snapshot.rows.find((row) => row.id === rowId);
    ...
    if (targetDateColumnId) {
      await this.ensureAutoGallery(
        ...
        this.options.warn(`Gallery 自动建 View 失败：${errorMessage(cause)}`)
      );
    }
  ...
  private ownedAttachment(input: {
    ...
    for (const association of Object.values(ledger.associations)) {
      ...
      const value = snapshot?.rows.find(
        (row) => row.id === association.rowId
  ...
  private async resolveGalleryAttachment(sourceRef: GalleryMediaSourceRef) {
    try {
      ...
      const galleryItemId =
        ledger.aliases[transcriptLogicalKey(sourceRef)] ??
        // 旧 ledger 只写 seq/item；仅作读取迁移，不再生成这种碰撞域。
        ledger.aliases[`transcript:${sourceRef.assistantSeq}:${sourceRef.itemId}`];
