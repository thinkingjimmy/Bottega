...
export class BaseOwnerResolver {
  ...
  async identityForOwnerKey(ownerKey: string): Promise<BaseOwnerIdentity> {
    ...
    return {
      ...
      ownerInstanceId: current?.meta.ownerInstanceId ?? "",
      title: project.name,
      navigation: baseNavigationForProject(project, current?.meta.navigation),
    };
  }
  ...
  async resolveTargetForSection(sectionId: string) {
    ...
    if (this.store.peek(ownKey, own.ownerInstanceId)) return own;
    if (chat.projectId) {
      const projectKey = `project:${chat.projectId}`;
      if (this.store.peek(projectKey)) {
