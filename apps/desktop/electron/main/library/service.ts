/**
 * [INPUT]: Depends on settings, folder identity, process lock, typed LibraryError codes and explicitly registered content mounts.
 * [OUTPUT]: Owns one selected folder per profile, admits a configured root only when it is still present, adopts its identity once, and reports a folderless profile as unconfigured so onboarding is reached.
...
export class LibraryService {
  ...
  constructor(private settings: SettingsStore, private installationId: string) {}
