 ...
 * [OUTPUT]: Provides the cloud persistence portion of the single current Chat schema.
 * [POS]: Schema source folded into 0001; never executed as an upgrade.
 */
...
CREATE TABLE chat_classification_candidates (
  ...
  candidate_json TEXT NOT NULL, candidate_hash TEXT NOT NULL,
  receipt_json TEXT, state TEXT NOT NULL CHECK(state IN ('pending','conflicted','confirmed','committed'))
) STRICT;
