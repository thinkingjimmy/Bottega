/**
 * [INPUT]: Depends on current execution identity, complete Home capture, verified body cache and the sole Chat writers.
...
export function installExecutionPrefix(db: SqliteDatabase, reader: ChatRepositoryReader, writer: ChatRecordWriter, scope: SyncScope, deviceId: string,
  ...
  if (!head || !native || state?.deleted || head.executorDeviceId !== deviceId || head.executionEpoch !== action.executionEpoch ||
  ...
  const oldOutbox = readChatOutbox(db, scope, native.id).filter(row => ["chat", "message", "turn"].includes(String(row.entity_kind)) &&
    row.kind !== "initialize" && row.execution_epoch !== null && Number(row.execution_epoch) < head.executionEpoch);
  ...
  const branch = divergent ? archiveExecution(db, scope, head, native, oldOutbox, now) : null;
  ...
  db.prepare("UPDATE chats SET cloud_last_committed_executor_device_id=? WHERE id=?").run(head.lastCommittedExecutorDeviceId, native.id);
  for (const row of oldOutbox) {
  ...
  return { chatId: native.id, executionEpoch: head.executionEpoch, bodyRevision: head.bodyRevision, branchId: branch?.branchId ?? null };
}
