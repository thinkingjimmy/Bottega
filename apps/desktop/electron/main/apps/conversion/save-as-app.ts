...
export class SaveAsAppService {
  ...
  async saveAsApp(input: SaveAsAppInput): Promise<AppRecord> {
    const normalized = assertSaveAsAppInput(input);
  ...
  private async execute(
    ...
    if (!reached("save-as-app", intent, "promoted")) {
  ...
  private async runPrePromotionPhases(
    ...
    if (!reached("save-as-app", intent, "record-created")) {
      ...
      if (!existing) {
        /* generation v2 必须先 seal bytes 再写 AppStore：文件先落盘、record 后
           提交，于是 record 在场即证明 scaffold 已在场，重放无需再写一遍。 */
    ...
    if (!reached("save-as-app", intent, "project-ensured")) {
  ...
  private async reconcilePromotionLink(
    ...
    if (reached("save-as-app", intent, "promoted")) return intent;
