/**
 * [INPUT]: Depends on Electron session, Node http/fs/path, http-proxy, generation bindings, the transport lifecycle in gateway-server.ts, and the frozen assets in gateway-legacy-sdk.ts
 * [OUTPUT]: Provides AppGateway with fixed-origin static/proxy/Base GUI hosting, one decoded pathname per request whose every rejection releases its lease, transport-bound proxy/WS leases, per-preview opaque origins, self-only generic egress CSP, nonce-bound static injection, and pre-publication worker/cache cleanup
...
import { GatewayHttpServer } from "./gateway-server";
import { GatewayFrozenAssets } from "./gateway-legacy-sdk";
...
export class AppGateway {
  ...
  constructor(
    ...
    this.transport = new GatewayHttpServer(join(userData, "apps-settings.json"));
  ...
  getOrigin(appId: string) {
    return `http://${appId}.localhost:${this.transport.port}`;
  }

  getSurfaceOrigin(appId: string, surfaceId: string) {
    return `http://${surfaceId}.${appId}.localhost:${this.transport.port}`;
  }
  ...
  private routeFromOrigin(value: string) {
    try {
      ...
      if (
        url.protocol !== "http:" ||
        Number(url.port) !== this.transport.port ||
        !identity
      ) {
        return undefined;
      }
  ...
  getServerInjectionJavascript() {
    return this.assets.serverInjectionJavascript();
  }

  allocateUpstreamPort() {
    return this.transport.allocateUpstreamPort();
  }
  ...
  private async handleRequest(
    ...
    /* 路径只解一次，而且在取 lease 之前：一份路径、一处判据，「解不出来」这
       条分支也就只剩一个出口。 */
    ...
    if (!leaseId) {
      if (
        route.type === "base-gui" &&
        pathname &&
        (isApiPath(pathname) || isSdkPath(pathname))
      ) {
        this.respondBaseGuiError(
          response,
          410,
    ...
    /* 400 也必须归还 lease：一个畸形 URL 就能把这个 App 的 countApp 永久钉在
       非零上，之后每一次 cutover/删除都要白等满 30 秒再超时。 */
    if (pathname === null) {
      release();
    ...
    if (route.type === "base-gui") {
      ...
      if (isSdkPath(pathname)) {
        try {
          await this.assets.serveLegacySdk(pathname, request, response);
        } finally {
          release();
        }
        return;
    ...
    try {
      await serveGatewayFile({
        ...
        baseGuiCsp: route.type === "base-gui"
          ? `${BASE_GUI_CSP}; frame-src http://*.${route.binding.appId}.localhost:${this.transport.port}`
          : BASE_GUI_CSP,
        reactGrab: this.assets.reactGrab,
      });
    } finally {
      release();
    }
