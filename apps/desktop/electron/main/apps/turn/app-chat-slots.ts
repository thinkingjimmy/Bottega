...
export class AppChatSlots {
  ...
  editAppIdOf(chatId: string) {
    const context = this.dependencies.chats.getMetadata(chatId)?.context;
    return context?.kind === "app-edit" ? context.appId : undefined;
  }

  async reconcile(): Promise<void> {
    /* 全量清单读一次：每台 App 各读一次只会把 O(App) 变成 O(App×Chat)。 */
    const canonical = this.dependencies.chats
      .list()
    ...
    for (const app of this.dependencies.apps.list()) {
      ...
      if (!project) continue;
      const chats = canonical.filter((chat) => chat.projectId === project.id);
