...
type SealedPackageReceipt = PackageDigestSet &
  Readonly<{
    root: string;
...
/** 被剥离的子树不进包，其内部结构与包契约无关：只审查真正入选的路径。 */
export async function inspectPackage(root: string): Promise<PackageInspection> {
  const canonicalRoot = await realpath(root);
  const files: PackageInspection["files"] = [];
  const ignored: string[] = [];
  /* ignored 是给人看的报告，不是账本：一棵 vendored 树能顶出几万条，
     UI 与校验都只需要「有东西被剥离」这一事实和头几个例子。 */
  const ignore = (path: string) => {
    if (ignored.length < IGNORED_REPORT_LIMIT) ignored.push(path);
  };
  let totalBytes = 0;
  let sourceBytes = 0;
  let sourceFiles = 0;
...
export async function inspectPackageDigests(
  ...
  return projectionDigests(
    root,
    manifest,
    await inspectProjections(root, manifest, inspected)
  );
}

async function projectionDigests(
  root: string,
  manifest: AppManifest,
  projections: PackageProjections
): Promise<PackageDigestSet> {
...
export async function sealPackageArtifact(input: {
  ...
  /* 同一棵树只走一遍：摘要复用这份 projection，而不是自己再扫一次盘。 */
  const projections = await inspectProjections(input.source, input.manifest);
  const expected =
    input.expected ??
    (await projectionDigests(input.source, input.manifest, projections));
...
type PackageProjections = {
  source: ProjectionFile[];
  runtime: ProjectionInspection;
};
