...
export class AppServerDataCutover implements AppServerCutoverPort {
  ...
  async prepare(input: {
    ...
    /* 上一条命已经走到 prepared：决定不再重复推进，但等待与构造必须原样
       重来一遍——世界在崩溃期间是会动的。 */
   ...
   *
   * 逐条独立收口：一条 cutover 判不动是那个 App 的事，不能让它把其余 App
   * 一起拖进启动崩溃循环。failed 的那条交回调用方去关它自己的 lifecycle gate。
   */
  async reconcile() {
    this.require();
    const settled = {
    ...
    for (const cutover of this.ledger.listUnsettled()) {
      try {
        await this.settleOne(cutover, settled);
      } catch (cause) {
        settled.failed.push({
          appId: cutover.appId,
          generationBuildId: cutover.generationBuildId,
