 ...
 * [OUTPUT]: Provides AppProcessCustodyJournal: an unreadable file is quarantined and rebuilt empty, the guardian walks intent→owned→activation-authorized→activated→released under monotonic CAS, plus pre-owned abort, quarantine, and per-generation drain counts
 * [POS]: The durable record of apps server process custody; the process is the only truth, and the gateway consumes nothing but an activated identity
 */
...
export class AppProcessCustodyJournal {
  ...
  /* 读不动的账本按「不存在」处理：隔离留证、空态重建。它记的是「谁曾被授权
     动这份数据」，读不出来时唯一诚实的做法是承认自己什么都不知道——而不是
     让整个产品卡在启动崩溃循环里。 */
  initialize() {
