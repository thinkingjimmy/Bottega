/**
 * [INPUT]: Depends on the shared AppGuiSurfaceTransitionCohort contract and live logical/runtime surface identities
...
export class AppGuiSurfaceCohort {
  ...
  join(member: Omit<AppGuiSurfaceTransitionMember, "state">) {
    ...
    return this.mutate((members) => {
      const joined = { ...member, state: "staging" as const };
      members.push(joined);
      return joined;
    });
  }
  ...
  ready(logicalSurfaceId: string) {
    return this.update(logicalSurfaceId, (member) =>
      member.state === "staging" ? { ...member, state: "ready" as const } : member
    );
  }
