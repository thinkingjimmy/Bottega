 ...
 * [OUTPUT]: Provides AppDataCutoverLedger; Non-v2 file fail closed, not moved, epoch proprietary single-mode, closed source/target fence, generationBuildId-level abort/released tombstone and non-terminal lists for re-booting
 * [POS]: The server mutable data of apps is written by the sole author (§3.4); Code generation does not have a physical root directly, and the switching sequence is driven by an app-server-cutover
 */
...
const cutoverSchema = z.object({
  ...
  phase: z.enum(["admission-close", "source-drain", "target-build", "cas", "done"]),
}).strict();
...
export class AppDataCutoverLedger {
  ...
  /** `open` 期间的进度标记；不改变决定，只让重启知道走到哪一步。 */
  advancePhase(
    generationBuildId: string,
    expectedRevision: number,
    phase: AppDataCutoverRecord["phase"]
  ) {
    return this.mutateCutover(generationBuildId, expectedRevision, (cutover) => {
      if (cutover.disposition !== "open") {
        throw new Error("只有 open 的 cutover 可推进 phase");
      }
      cutover.phase = phase;
    });
  }
  ...
  /**
   * 首次 abort 即使尚无 cutover record 也要留 build 级 tombstone：
   * 否则一条「已经决定放弃」的 build id 还能被迟到的 prepare 重新开张。
   */
  retireBuild(generationBuildId: string) {
    return this.file.mutate((state) => {
      if (!state.retiredBuildIds.includes(generationBuildId)) {
        state.retiredBuildIds.push(generationBuildId);
      }
      return generationBuildId;
    });
  }
