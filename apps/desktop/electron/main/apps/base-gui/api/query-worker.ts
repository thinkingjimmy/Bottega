/**
 * [INPUT]: Depends on Node worker_threads, immutable registered Base snapshots carrying their main-measured byte size, and the Query V1 pure executor
 * [OUTPUT]: Provides the isolated worker message loop for register, evict, and absolute-deadline query operations that yield a typed query_timeout before main terminates the worker
 * [POS]: Off-main Query V1 execution leaf; it owns no authority and cannot read Base storage directly
 */

import { parentPort } from "node:worker_threads";
import type { BaseGuiQueryRequestV1 } from "../../../../../shared/app-gui/query";
import type { BaseSnapshot } from "../../../../../shared/bases-ipc";
import { executeBaseGuiQueryV1 } from "./query-v1";
...
/* 字节体量在 main 注册快照时量过一次，这里只存不再重算。 */
const snapshots = new Map<string, Readonly<{ snapshot: BaseSnapshot; rowsBytes: number }>>();
