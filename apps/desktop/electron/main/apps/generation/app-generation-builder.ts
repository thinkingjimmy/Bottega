...
import { removePackageArtifact, sealPackageArtifact } from "../share/package/package-contract";
import {
...
export class AppGenerationBuilder {
  ...
  private async bindBaseGuiCapability(
    ...
    /* 走到这里 pending 一定已由前面的 staging 路径写好——Base GUI 参与者从来
       不是第一个落笔的人，所以不存在「没有 pending 但已批准」这条岔路。 */
    const pending = record.generationBinding.pending!;
    const nextPending = {
      ...pending,
      baseGuiDecision: decisionPointer(decision),
      extensionState: pending.state,
    };
