/**
 * [INPUT]: No platform dependencies; consumed by native and browser shortcut adapters
 * [OUTPUT]: Exports five locale catalogs, getShortcutCopy and shortcutText for shared shortcut settings, recorder feedback and action labels
 * [POS]: Five-language shared keyboard shortcut vocabulary; key glyphs (⌘/Ctrl) are rendered by code, not translated here
 */
...
export const settingsShortcutsFr: SettingsShortcutsCatalog = {
  ...
  hints: {
    taskPanel:
      "Disponible dans toutes les applications quand l’affichage des tâches en haut de l’écran est activé.",
  ...
  errors: {
    ...
    reservedCombo:
      "Cette combinaison est réservée par l’application ou le système",
    unsupportedKey:
      "Utilisez une lettre, un chiffre, une ponctuation ou F1–F12",
  },
...
export const settingsShortcutsEs: SettingsShortcutsCatalog = {
  ...
  hints: {
    taskPanel:
      "Disponible en todas las aplicaciones mientras esté activado el estado de tareas en la parte superior.",
  ...
  errors: {
    ...
    altReserved:
      "Las combinaciones con Alt/Option están reservadas para el sistema",
    reservedCombo:
      "Esta combinación está reservada por la aplicación o el sistema",
...
export function getShortcutCopy(locale: string) {
  const messages: Record<string, SettingsShortcutsCatalog> = {
    en: settingsShortcutsEn,
    "zh-CN": settingsShortcutsZhCN,
    ja: settingsShortcutsJa,
    fr: settingsShortcutsFr,
    es: settingsShortcutsEs,
  };
  return messages[locale] ?? settingsShortcutsEn;
}
export function shortcutText(template: string, values: Record<string, string>) {
