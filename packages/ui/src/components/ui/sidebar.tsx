...
function SidebarProvider({
  ...
  /* 内建 ⌘/Ctrl+B 监听的开关。默认开，保持既有消费者行为；宿主若有
     自己的中央快捷键分派（如 desktop 的可改绑注册表），传 false 交权，
     否则同一个键会被两处各自 preventDefault。 */
...
function Sidebar({
  ...
  const sidebarDescription = useUiText(
    "sidebarDescription",
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          "relative w-(--sidebar-width) bg-transparent transition-[width] duration-200 ease-linear group-has-data-[resizing=true]:transition-none",
      ...
      <div
        ...
        data-side={side}
        className={cn(
          "fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-200 ease-linear group-has-data-[resizing=true]:transition-none data-[side=left]:left-0 data-[side=left]:group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)] data-[side=right]:right-0 data-[side=right]:group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)] md:flex",
...
function SidebarTrigger({
  ...
  const label = useUiText("toggleSidebar", "Toggle Sidebar")
...
function SidebarRail({
  ...
  const toggleLabel = useUiText("toggleSidebar", "Toggle Sidebar")
  const resizeLabel = useUiText("resizeSidebar", "Resize sidebar")
  const resizeHint = useUiText(
    "resizeSidebarHint",
  ...
  if (canResize && !open && !resize.active) return null
  ...
  return (
    <button
      ...
      aria-label={canResize ? resizeLabel : toggleLabel}
      tabIndex={-1}
      onClick={(event) => {
        onClick?.(event)
        if (event.defaultPrevented) return
        if (!canResize) toggleSidebar()
      }}
      onPointerDown={(event) => {
        onPointerDown?.(event)
        if (!event.defaultPrevented) resize.start(event)
      }}
      onPointerMove={(event) => {
        onPointerMove?.(event)
        if (!event.defaultPrevented) resize.move(event)
      }}
      onPointerUp={(event) => {
        onPointerUp?.(event)
        resize.finish(event)
      ...
      title={canResize ? resizeHint : toggleLabel}
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 sm:flex ltr:-translate-x-1/2 rtl:-translate-x-1/2",
        "in-data-[side=left]:cursor-w-resize in-data-[side=right]:cursor-e-resize",
        "[[data-side=left][data-state=collapsed]_&]:cursor-e-resize [[data-side=right][data-state=collapsed]_&]:cursor-w-resize",
        "group-data-[collapsible=offcanvas]:translate-x-0",
        "[[data-side=left][data-collapsible=offcanvas]_&]:-right-2",
        "[[data-side=right][data-collapsible=offcanvas]_&]:-left-2",
        canResize && "touch-none cursor-col-resize!",
        className
      )}
      {...props}
    />
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        /* 显现通道用 :has(:focus-visible) 而非 focus-within：鼠标点一下行内链接，
           focus 就留在它身上，focus-within 会让浮层从此赖着不走（指针早已离开侧栏）。
           但这只堵住「点行」一路——菜单开关那一路 Radix 是程序化交还 focus，
           Chromium 会让 focus-visible 沿继承链一路为真，选择器救不了，
           须由 usePointerOpenedMenu 在关闭时不交还 focus 来治因。 */
        showOnHover &&
