...
import { CheckIcon, ChevronRightIcon, CopyIcon, ExternalLinkIcon, TriangleAlertIcon } from "lucide-react";
  ...
  return (
    <div className={cn(
      ...
      <div className="space-y-1 text-sm" role="alert" aria-atomic="true">
        ...
        {children}
        {report && <ReportIssueAction {...report} />}
      </div>
      {copy.diagnostic && labels && (
...
export function ReportIssueAction({ label, run }: { label: string; run(): void }) {
  return <Button className="touch-manipulation" onClick={run} size="sm" type="button" variant="outline">
    {label}<ExternalLinkIcon aria-hidden="true" data-icon="inline-end" />
  </Button>;
}
