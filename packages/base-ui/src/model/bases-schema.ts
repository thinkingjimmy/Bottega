 ...
 * [POS]: Shared Base semantic kernel; Shared Base zod-validation layer; kept in lockstep with the types and constants bases-ipc owns (mirroring how agent-schema relates to agent-ipc), so only main needs zod to validate IPC input, not the renderer
 */

import { z } from "zod";
import { CHART_TYPES } from "./chart-payload";
import { BASE_AGGREGATIONS } from "../compute/base-aggregations";
import { BASE_FORMULA_EXPRESSION_LIMIT } from "../compute/base-formula";
import { baseAttachmentValueSchema } from "../attachments/gallery-attachments";
import type {
  BaseCellValue,
  BaseColumn,
  BaseRow,
...
import { BASE_CELL_STRING_LIMIT, BASE_CHAT_ID_PATTERN, BASE_COLUMN_LIMIT, BASE_COLUMN_WIDTH_MAX, BASE_COLUMN_WIDTH_MIN, BASE_DELETE_LIMIT, BASE_ENTITY_ID_PATTERN, BASE_FILTER_DEPTH_LIMIT, BASE_FILTER_NODE_LIMIT, BASE_INSERT_LIMIT, BASE_NAME_LIMIT, BASE_OWNER_KEY_PATTERN, BASE_ROW_LIMIT, BASE_SELECT_OPTION_LIMIT, BASE_VIEW_LIMIT, BASE_XLSX_ISSUE_REASONS, CHART_ITEM_LIMIT, type BaseMeta, type BaseView } from "./bases-ipc";
