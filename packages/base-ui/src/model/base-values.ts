 ...
 * [OUTPUT]: Provides Base value types, source-device-only image availability, canonical cell text, deleted-relation text, option deduplication and strict dates.
 * [POS]: Shared Base semantic kernel; Shared semantic core for Base column and cell values; bases-ipc re-exports a stable subset so main and renderer consume identical types
 */
...
export type BaseAttachmentValue = {
  ...
  height: number;
  revision: string;
  localAvailability?: { sourceDeviceId: string };
};
