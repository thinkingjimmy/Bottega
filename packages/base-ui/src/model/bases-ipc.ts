/**
 * [INPUT]: Depends on isolated owner-key types, Base value/view/attachment contracts and the canonical BaseCellContext evaluation kernel
 * [OUTPUT]: Owner-aware Base/navigation IPC with local-only, preparing, pending, candidate and confirmed save-state projections.
 * [POS]: Shared Base semantic kernel; The shared Base wire and projection authority used by main, renderer, and the builtin-tool server; projection never manufactures its own partial evaluation context
 */

import type { BaseOwner } from "./owner-key";
...
} from "../attachments/gallery-attachments";
import type {
...
import type { BaseNavigation } from "../metadata/placement-facts";

export { baseCellText, dedupeSelectOptions, formatBaseDate, isBaseAttachmentValue, parseBaseDate } from "./base-values";
export { cellValue, createBaseCellContext, isBaseRelationLabelColumn } from "../compute/base-cell-value";
export type { BaseCellContext } from "../compute/base-cell-value";
export {
...
export type * from "../attachments/gallery-attachments";
export type {
  BaseAggregation,
...
export { BASE_OWNER_KEY_PATTERN } from "./owner-key";
...
export type BasesBridgeApi = {
  images?: import("../attachments/native-images").BaseImagesBridge;
  ...
  getRecovery?(input: { ownerKey: string }): Promise<{
    files: string[];
    reason: "content" | "project-missing" | "owner-incarnation-changed";
  } | null>;
