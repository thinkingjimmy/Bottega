 ...
 * [OUTPUT]: Provides BaseViewTabs; Single VIEW_TYPES directory drives six standard view tabs with added menus, taller on coarse pointers
...
import type { BaseView, BaseViewConfig } from "@ai-chat/base-ui/model/bases-ipc";
import { BASE_VIEW_LIMIT } from "@ai-chat/base-ui/model/bases-ipc";
import {
        ...
        {orderedViews.map((view) =>
          view.id === renameViewId ? (
            <InlineNameInput
              ...
              autoFocus
              className="h-6 w-32 shrink-0 text-xs pointer-coarse:h-8"
              name={view.name}
...
function ViewTab({
  ...
  return (
    <div
      ...
      className={cn(baseTabShellClass(active), "h-6 shrink-0 gap-1 pointer-coarse:h-8")}
      onClick={onSelect}
