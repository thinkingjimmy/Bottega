 ...
 * [OUTPUT]: Projects Gallery rows with source-device availability, date groups, visual rows, pinned identities and virtual-row lookup.
...
export function projectGalleryRows(
  ...
  const items = projection.rows.flatMap((row): GalleryItem[] => {
    ...
    return [{
      ...
      ...(title ? { title } : {}),
      sourceRef,
      ...(attachment.localAvailability ? { localAvailability: attachment.localAvailability } : {}),
    }];
  });
