 ...
 * [OUTPUT]: Provides bounded Gallery/attachment contracts, 50 MB uploadable values and 64 MiB source-device-only automatic originals without physical locators.
 * [POS]: Shared Base semantic kernel; The only source of truth for shared/bases attachment protocols; BaseStore, ingestion, preload and renderer are shared, with no paths or workspace lease in the structure
 */
...
import { BASE_OWNER_KEY_PATTERN } from "../model/owner-key";

export const BASE_ATTACHMENT_BYTE_LIMIT = 50_000_000;
export const BASE_LOCAL_IMAGE_BYTE_LIMIT = 64 * 1024 * 1024;
...
export const localImageAvailabilitySchema = z.object({ sourceDeviceId: id }).strict();
...
export const baseAttachmentValueSchema = z
  .object({
    ...
    byteLength: z.number().int().positive().max(BASE_LOCAL_IMAGE_BYTE_LIMIT),
    width: z.number().int().positive().max(32_768),
    height: z.number().int().positive().max(32_768),
    revision: z.string().min(1).max(256),
    localAvailability: localImageAvailabilitySchema.optional(),
  })
  .strict().superRefine((value, context) => {
    if ((value.byteLength > BASE_ATTACHMENT_BYTE_LIMIT) !== Boolean(value.localAvailability)) {
      context.addIssue({ code: "custom", message: "Only oversized local images carry source-device availability" });
    }
  });
