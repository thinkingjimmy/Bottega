 ...
 * [OUTPUT]: Provides dual source refs, optional Subagent preview identity, canonical keys, source/destination logistics and bounded projection/index results.
 * [POS]: Shared Base semantic kernel; The Gallery's media security borders; Any renderer visible structure does not contain a work area path or read lease
 */
...
export const galleryThumbnailInputSchema = z
  .object({
    ...
    subagentId: z.string().min(1).max(256).optional(),
  })
  .strict();
