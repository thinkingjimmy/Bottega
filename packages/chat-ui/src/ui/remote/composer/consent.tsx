 ...
 * [OUTPUT]: Explicit session-bound Full Access confirmation and cancellation without a global grant.
 * [POS]: UI consent boundary; main independently validates the returned scope before dispatch.
 */
import { useEffect, useRef, useState } from "react";
import { remoteConsentMatches, type RemoteFullAccessConsent } from "@ai-chat/cloud-protocol/remote/input/model";
...
export function useRemoteConsent(store: RemoteDraftStore, locale: string, computer: string, identity: string) {
  const [pending, setPending] = useState<RemoteFullAccessConsent | null>(null);
  const settle = useRef<((value: RemoteFullAccessConsent | null) => void) | null>(null);
  const finish = (value: RemoteFullAccessConsent | null) => { const done = settle.current; settle.current = null; setPending(null); done?.(value); };
  ...
  const confirm = async (scope: Omit<Extract<RemoteFullAccessConsent, { version: 1 }>, "version">) => {
    const consent = store.snapshot().consent;
    if (remoteConsentMatches(consent ?? undefined, scope)) return consent;
    settle.current?.(null);
    const value: RemoteFullAccessConsent = { version: 1, ...scope };
    setPending(value);
    return new Promise<RemoteFullAccessConsent | null>(resolve => { settle.current = resolve; });
  };
