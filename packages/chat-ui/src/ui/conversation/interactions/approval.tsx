/**
 * [INPUT]: A portable approval projection, its decision choices and the injected host translator.
 * [OUTPUT]: Provides ChatApprovalCard — the shared approval card with its command/diff detail, choice rows and busy states.
 * [POS]: The approval half of conversation/interactions, rendered by native and remote hosts alike; no IPC or execution authority.
 */
import { ArrowRightIcon, LightbulbIcon, ShieldAlertIcon } from "lucide-react";
import { Button } from "@ai-chat/ui/components/ui/button";
import { SlimScroller } from "@ai-chat/ui/components/ui/slim-scroller";
import { cn } from "@ai-chat/ui/lib/utils";
import type { ApprovalDecision as AgentApprovalDecision, ApprovalRequest as AgentApprovalRequest } from "./model";
import { useInteractionTranslation } from "./translation";
...
export function ChatApprovalCard({
  ...
  const { t } = useInteractionTranslation();
  ...
  return (
    ...
    >
      ...
      {approval.diff && <SlimScroller className="mt-2 max-h-64 overflow-auto rounded-lg border bg-muted/30 p-2" aria-label={t("chat.composer.approval.details")}>
        <pre className="text-xs leading-5">{approval.diff.split("\n").map((line, index) => <span key={index} className={cn("block", line.startsWith("+") && "text-green-700 dark:text-green-400", line.startsWith("-") && "text-red-700 dark:text-red-400")}>{line || " "}</span>)}</pre>
      </SlimScroller>}
