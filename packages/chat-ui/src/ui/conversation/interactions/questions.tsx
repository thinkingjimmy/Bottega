/**
 * [INPUT]: A portable user-input request, its queue position and the injected host translator.
 * [OUTPUT]: Provides ChatUserInputSelector — sequential question answering with required/other answers, dirty reporting and expiry.
 * [POS]: The question half of conversation/interactions; an unrepresentable question stays unanswered rather than guessed.
 */
...
import type { PendingQuestion as PendingUserInputState } from "./model";
...
function ChatUserInputQuestion({
  ...
  useEffect(() => { onDirtyChange?.(Boolean(text.trim()) || selected.length > 0); }, [onDirtyChange, text, selected]);
  if (!question) return null;
  const errorText =
