/**
 * [INPUT]: Portable user/reply summaries and canonical outline entries.
 * [OUTPUT]: Provides outlineEntries, outlineMinimapEntries and activeOutlineIndex in transcript order.
 * [POS]: The outline's pure model, consumed by outline.tsx; no DOM and no paging.
 */
export type OutlineMessage = { id: string; role: "user" | "assistant" | "notice"; content: string; isError?: boolean; attachments?: readonly { filename: string }[] };
export type CanonicalOutlineItem = { messageId: string; seq: number; role: OutlineMessage["role"]; text: string };
...
export function outlineEntries(messages: readonly OutlineMessage[]): OutlineEntry[] {
...
export function outlineMinimapEntries(
  ...
  canonicalItems: readonly CanonicalOutlineItem[] = []
): OutlineEntry[] {
