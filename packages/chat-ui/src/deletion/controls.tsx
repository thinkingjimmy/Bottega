/**
 * [INPUT]: Depends on the injected deletion controller, confirmed Chat revision, connectivity and shadcn Dialog/Button.
...
export function ChatDeletion({ head, session, locale, connected, disabled = false, deleted = false, durable = false, initiallyOpen = false, hideTrigger = false, onUnresolvedChange, onDismiss }: {
  head: CloudChatHead; session: ChatDeletionController; locale: string; connected: boolean; disabled?: boolean; deleted?: boolean;
  initiallyOpen?: boolean; hideTrigger?: boolean; durable?: boolean; onUnresolvedChange?(unresolved: boolean): void; onDismiss?(): void;
}) {
  ...
  const canSubmit = connected || durable;
