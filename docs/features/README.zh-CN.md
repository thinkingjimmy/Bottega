...
- 同步始终可选：不登录也能完整使用；登录本身不上传任何内容，直到你确认第一次同步。
...
- 一个账号一个加密空间。另一台电脑选择自己的 Bottega 文件夹，用同一账号登录，再输入同一密码加入。
...
- 远程操作在服务端开启后，可从 Web 驱动已登录的桌面：选择电脑与 Agent、发送消息、查看实时输出、停止、批准或拒绝权限请求、回答追问、插话与追加消息。目标必须在线、已解锁、协议版本一致且执行准备就绪。
- 远程操作是服务端开关，而不是应用设置或隐藏按钮。关闭期间，浏览器里的 Chat 为只读；阅读转录和实时观看运行中的轮次仍然可用。
...
- 更换版本前备份完整应用数据目录。0.1.5 使用新的本地存储布局，从 0.1.4 或更早版本升级需要空白应用数据目录，操作方式见[升级说明](../getting-started/README.zh-CN.md#升级到-015)。
