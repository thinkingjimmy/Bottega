...
重装 Bottega 或换电脑后，在首次设置中选择复制来的文件夹，已保存的内容即可离线查看。外部 Project 文件夹需要重新关联，App 需要在新电脑上重新授权；登录账号并输入同步密码后可重新连接 Cloud Sync。旧备份与云端内容不同时，云端保留当前内容，本机对话差异会成为独立 Chat。恢复后的 Agent 会话只有在保存的历史边界可验证时才续接，否则 Bottega 会用保存的历史开启新会话并明确说明。

完成首次设置后，文件夹位置固定。需要换位置时，请退出 Bottega，完整复制文件夹，重装后选择副本。Settings › General 会显示当前位置，并可在文件管理器中打开。修改或删除文件夹内的转录文件，不会反向修改或删除 Bottega 中的对话。
...
首次启动时，Bottega 会先询问只在这台电脑上使用还是连接已有账号，然后请你选择 Bottega 文件夹。选择空文件夹即可从头开始，选择已有的 Bottega 文件夹则重新打开其中的内容。任何一次安装都要经过这一步，从旧版本升级也一样。
...
如果当前安装的是 0.1.0 或 0.1.1，请先手动下载并安装一次 [0.1.5](https://github.com/thinkingjimmy/Bottega/releases/tag/v0.1.5)。旧版本的更新器存在问题，修复会在安装新版二进制后生效；Windows 继续通过手动下载安装包升级。
...
云同步是可选功能，在你主动开启前保持关闭。纯本地使用不需要账号，也不会积累上传队列。

- **在浏览器里登录。** Bottega 打开系统默认浏览器完成 Google 登录，你在浏览器里批准该请求，桌面应用再接续会话。登录方式只有 Google，Bottega 不会索取该密码。
- **设置同步密码。** 第一台电脑设置一个独立的同步密码，至少 8 个字符并同时包含英文字母和数字。它与 Google 密码无关；加密内容的密钥在你自己的设备上用 Argon2id 派生，内容在离开电脑前用 XChaCha20-Poly1305 封装，服务端只保存密文。
- **没有密码找回。** 没有恢复密钥、没有其他设备批准、没有重置入口。密码丢失且没有任何已登录设备还能解密时，仅存在于云端的内容无法恢复。开启同步前 Bottega 会明确说明这一点，并要求你确认。
...
远程操作在服务端开启后，可以从 Cloud Web 或手机浏览器驱动一台在线且已解锁的已登录桌面：选择电脑与 Agent、发送消息、实时查看回复、停止、批准或拒绝权限请求、回答追问、插话与追加消息。远程操作是服务端开关而不是应用设置，在 Bottega 开启前保持关闭。关闭期间，浏览器里的 Chat 为只读；阅读转录和实时观看运行中的轮次仍然可用。
...
首次启动时，先选择只在这台电脑上使用还是连接已有账号，再选择 Bottega 文件夹，并等待 Bottega 检测本机 CLI。Skills 与长期记忆是同一向导中的可选步骤，之后也可以在设置中调整。文件夹与至少一个后端就绪后，即可创建任务，并在发送首条消息前选择 Agent。
